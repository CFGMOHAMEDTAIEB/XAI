...
def pack_container(metadata: dict, payload: bytes) -> bytes:
    md = dict(metadata)
    md.update({"format_version":FORMAT_VERSION, "payload_size":len(payload), "payload_sha256":sha256(payload)})
    raw = _canonical(md)
    if len(raw) > MAX_METADATA: raise FormatError("metadata too large")
    return PREFIX.pack(MAGIC, FORMAT_VERSION, len(raw)) + raw + hashlib.sha256(raw).digest() + payload
...
def unpack_container(blob: bytes, max_output_size=MAX_OUTPUT_DEFAULT):
    ...
    if version != FORMAT_VERSION: raise FormatError("unsupported format version")
    ...
    if md["format_version"] != FORMAT_VERSION: raise FormatError("metadata version mismatch")
