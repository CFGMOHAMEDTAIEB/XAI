...
class LocalRotaryAttention(nn.Module):
    ...
    def forward(self, x, positions, cache=None, max_cache=256, numerical_debug=False, layer=None):
        b,t,c=x.shape
...
class GatedBlock(nn.Module):
    ...
    def forward(self,x,positions,cache=None,max_cache=256,numerical_debug=False,layer=None):
        a,cache=self.attn(self.n1(x),positions,cache,max_cache,numerical_debug,layer);x=x+a
        ...
        out=self.proj(value*torch.nn.functional.silu(gate))
...
class CausalByteTransformerV2(nn.Module):
    ...
    def forward(self,tokens:torch.Tensor,hidden:Any=None):
        ...
        for i,block in enumerate(self.blocks):
            x,cache=block(x,positions,layers[i] if layers else None,self.config.context_length,self.numerical_debug,i);new.append(cache)
        x=self.norm(x)
