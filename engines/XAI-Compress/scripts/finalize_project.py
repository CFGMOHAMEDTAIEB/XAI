...
def lossless_benchmark(samples):
    ...
    paired_name=str(path.relative_to(ROOT))+"[:2048]"
    for name,(enc,dec) in methods.items():
        pair_blob,pair_ct=timed(enc,data);pair_restored,pair_dt=timed(dec,pair_blob);pair_passed=sha(data)==sha(pair_restored)
        rows.append({"file":paired_name,"category":category,"codec":name,"status":"PASS" if pair_passed else "FAIL","original_bytes":len(data),"artifact_bytes":len(pair_blob),"actual_bpb":8*len(pair_blob)/len(data),"compression_ratio":len(data)/len(pair_blob),"compression_MB_s":len(data)/(1<<20)/max(pair_ct,1e-12),"decompression_MB_s":len(data)/(1<<20)/max(pair_dt,1e-12),"peak_RSS_MB":rss(),"sha256_pass":pair_passed})
...
def main():
    ...
    valid_lossless=[r for r in lossless if r["status"]=="PASS"];best=min(valid_lossless,key=lambda r:r["actual_bpb"]);gru_row=next(r for r in lossless if r["codec"]=="xai-gru-rans");paired=[r for r in lossless if r["file"]==gru_row["file"] and r["codec"] not in {"xai-static","xai-gru-rans"}]
