...
def test_v2_training_smoke_uses_separate_namespace(tmp_path):
    ...
    last_stable = output.with_name("last_stable.pt")
    assert last_stable.is_file()
    stable_model, stable_object = load_checkpoint(last_stable, "cpu")
    assert stable_model.config.architecture_id == "causal-byte-transformer-v2"
    assert stable_object["metrics"]["checkpoint_role"] == "last_stable"
    assert "rng_state" in stable_object
...
def test_reduced_amp_scale_is_bounded_and_rejects_invalid_values():
    assert reduced_amp_scale(512) == 256
    assert reduced_amp_scale(1) == 1
    with pytest.raises(ValueError):
        reduced_amp_scale(float("nan"))
