...
def build_package(root: Path, staging: Path, username: str, slug: str) -> dict:
    ...
    if staging == root or root in staging.parents and not staging.name.startswith(".kaggle_build"):
    ...
    for relative in critical:
        packaged = staging / relative
        if packaged.is_file():
            hashes[relative] = hashlib.sha256(packaged.read_bytes()).hexdigest()
    (staging / "source-sha256.json").write_text(json.dumps(hashes, indent=2), encoding="utf-8")
