...
MAGIC = b"XAIC"
FORMAT_VERSION = 1
SUPPORTED_FORMAT_VERSIONS = {1, 2, 4}
...
def unpack_container(blob: bytes, max_output_size=MAX_OUTPUT_DEFAULT):
    ...
    if version == 4 and (md.get("mode") != "neural-lossy" or md.get("lossless") is not False):
        raise FormatError("XAIC v4 must be explicitly neural-lossy")
    return md, payload
