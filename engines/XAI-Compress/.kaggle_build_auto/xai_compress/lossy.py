...
LATENT_FREQUENCIES=struct.Struct(">256H")


def _rans_encode_latent(raw: bytes) -> bytes:
    counts=np.bincount(np.frombuffer(raw,dtype=np.uint8),minlength=256).astype(np.float64)
    cumulative=logits_to_cumulative(torch.from_numpy(np.log(counts+1.0)))
    frequencies=[cumulative[i+1]-cumulative[i] for i in range(256)]
    encoder=RANSEncoder()
    for symbol in raw:encoder.write_fast(cumulative,symbol)
    return LATENT_FREQUENCIES.pack(*frequencies)+encoder.finish()


def _rans_decode_latent(payload: bytes, length: int) -> bytes:
    if len(payload)<LATENT_FREQUENCIES.size+4:raise LossyCompressionError("truncated lossy rANS latent")
    frequencies=LATENT_FREQUENCIES.unpack_from(payload);cumulative=[0]
    for frequency in frequencies:cumulative.append(cumulative[-1]+int(frequency))
    if cumulative[-1]!=16384 or any(frequency<1 for frequency in frequencies):
        raise LossyCompressionError("invalid lossy latent frequency table")
    decoder=RANSDecoder(payload[LATENT_FREQUENCIES.size:])
    return bytes(decoder.read_fast(cumulative) for _ in range(length))
...
def compress_lossy_bytes(data:bytes,checkpoint,quality="medium",device="cpu"):
    ...
    step=QUALITY_STEPS[quality];q=torch.round(z/step).clamp(-32768,32767).short().cpu().numpy().astype("<i2",copy=False)
    payload=LATENT_HEADER.pack(w,h,q.shape[1],q.shape[2],step)+_rans_encode_latent(q.tobytes())
    md={"mode":"neural-lossy","lossless":False,"coder_version":"latent-rans14-v1","original_size":len(data),
...
def decompress_lossy_bytes(blob:bytes,checkpoint,device="cpu"):
    ...
    w,h,c,lh,step=LATENT_HEADER.unpack_from(payload)
    lw=(w+3)//4;expected=c*lh*lw*2
    raw=_rans_decode_latent(payload[LATENT_HEADER.size:],expected)
    if len(raw)!=expected:raise LossyCompressionError("lossy latent length mismatch")
    q=np.frombuffer(raw,dtype="<i2").reshape(1,c,lh,lw).copy()
