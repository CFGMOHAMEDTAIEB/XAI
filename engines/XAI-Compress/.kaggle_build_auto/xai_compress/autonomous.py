...
def kaggle_kernel_slug(candidate_key: str, fingerprint: str) -> str:
    key = re.sub(r"[^a-z0-9]+", "-", candidate_key.lower()).strip("-")
    prefix, suffix = "xai-compress-auto-", f"-{fingerprint[:10]}"
    key = key[:max(1, 50 - len(prefix) - len(suffix))].rstrip("-")
...
class KaggleBackend:
    ...
    def deploy_source(self) -> dict:
        ...
        build = self.root / ".kaggle_build_auto"
        environment["KAGGLE_BUILD_DIR"] = str(build)
        ...
        if completed.returncode:
            raise RuntimeError((completed.stdout + "\n" + completed.stderr).strip())
        manifest = json.loads((build / "source-sha256.json").read_text(encoding="utf-8"))
        return manifest

    def stage(self, candidate: dict, source_hashes: dict) -> tuple[Path, str]:
        # Kaggle rejects overlong kernel slugs with an opaque HTTP 400.
        slug = kaggle_kernel_slug(candidate["candidate_key"], candidate["fingerprint"])
        ...
        metadata = {
            "id": f"{username}/{slug}", "title": slug.replace("-", " ")[:50], "code_file": "run.py",
...
class AutonomousController:
    ...
    def run(self, once: bool = False) -> str:
        with self.lock:
            ...
            controller_errors = 0
            maximum_errors = int(self.config.payload["kaggle"].get("max_controller_errors", 3))
            while True:
                try:
                    ...
                    self.events.append(candidate=None, action="CONTROLLER_ERROR", reason=type(exc).__name__,
                                       result={"message": str(exc), "attempt": controller_errors,
                                               "maximum": maximum_errors})
                    if once:
                        raise
                    if controller_errors >= maximum_errors:
                        state = self.orchestrator.initialize()
                        state["status"] = "INFRASTRUCTURE_BLOCKED"
                        state["stop_reason"] = "CONTROLLER_ERROR_RETRY_LIMIT"
                        self.orchestrator.save_state(state)
                        self._persist_controller("STOPPED", result="CONTROLLER_ERROR_RETRY_LIMIT")
