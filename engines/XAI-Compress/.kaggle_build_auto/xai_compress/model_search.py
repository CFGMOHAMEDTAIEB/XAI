...
def extract_first_nonfinite_evidence(summary: dict) -> dict:
    """Recover the primary fail-fast event even when telemetry masks it later."""
    text = "\n".join(str(summary.get(key, "")) for key in ("exception", "traceback"))
    match = re.search(
        r"FIRST NON-FINITE STAGE\s*=\s*([^;\n]+);\s*epoch=(\d+);\s*step=(\d+)",
        text,
        re.IGNORECASE,
    )
    if not match:
    ...
    return {
        "first_non_finite_stage": stage,
        "tensor_name": stage.split(":", 1)[1] if ":" in stage else None,
        "epoch": int(match.group(2)),
        "step": int(match.group(3)),
        "primary_exception": match.group(0),
        "telemetry_masked_primary_failure": "same device" in text.lower(),
    }
...
class ModelSearchOrchestrator:
    ...
    def load_state(self) -> dict:
        ...
        changed = False
        registered_keys = {item["candidate_key"] for item in state.get("candidates", {}).values()}
        pending = state.setdefault("pending_candidates", [])
        for definition in self.config.payload["candidates"]:
            key = definition["key"]
            if key not in registered_keys and key not in pending:
                pending.append(key)
                changed = True
        budget = state["budget"]
    ...
    def supersede_unsubmitted_candidate(self, state: dict, candidate: dict, reason: str) -> dict:
        """Retire a never-submitted candidate without inventing a scientific failure."""
        if candidate.get("status") != "PENDING" or candidate.get("kaggle_kernel") or candidate.get("kaggle_version"):
            raise RuntimeError("only an unsubmitted pending candidate may be superseded")
        evidence = {
            "candidate_id": candidate["candidate_id"], "candidate_key": candidate["candidate_key"],
            "status": "SUPERSEDED_UNSUBMITTED", "reason": reason, "timestamp": utc_now(),
            "scientific_result": "NOT_RUN", "submission_budget_consumed": False,
        }
        candidate.update({"status": "SUPERSEDED_UNSUBMITTED", "decision": "INCONCLUSIVE",
                          "scientific_status": "NOT_RUN", "superseded": evidence, "updated_at": utc_now()})
        if candidate["candidate_id"] not in state["completed_candidates"]:
            state["completed_candidates"].append(candidate["candidate_id"])
        atomic_json(Path(candidate["candidate_dir"]) / "superseded_unsubmitted.json", evidence)
        atomic_json(Path(candidate["candidate_dir"]) / "status.json", candidate)
        self.save_state(state); self.update_leaderboard(state)
        return evidence

    def authorize_controller_infrastructure_repair(self, state: dict, reason: str, test_evidence: dict) -> dict:
        if state.get("status") != "INFRASTRUCTURE_BLOCKED" or state.get("running_candidate"):
            raise RuntimeError("controller repair requires a stopped infrastructure-blocked search")
        if not test_evidence.get("package_verified"):
            raise RuntimeError("controller repair requires verified package evidence")
        repair={"timestamp":utc_now(),"reason":reason,"test_evidence":test_evidence,
                "previous_status":state["status"],"submission_budget_consumed":False}
        state["status"]="ACTIVE";state["stop_reason"]=None
        atomic_json(self.paths.results/"controller_infrastructure_repair.json",repair)
        self.save_state(state);return repair
    ...
    def record_failure(self, state: dict, candidate: dict, stage: str, exception: BaseException | str,
        ...
        previous_failure = candidate.get("failure")
        history = candidate.setdefault("failure_history", [])
        if previous_failure and previous_failure not in history:
            history.append(previous_failure)
        failure["attempt"] = len(history) + 1
        ...
        failure_dir = self.paths.results / "failures"
        original_destination = failure_dir / f"{candidate['candidate_id']}.json"
        destination = original_destination if not original_destination.exists() else failure_dir / f"{candidate['candidate_id']}.attempt-{failure['attempt']}.json"
        candidate_destination = Path(candidate["candidate_dir"]) / ("failure.json" if failure["attempt"] == 1 else f"failure.attempt-{failure['attempt']}.json")
        failure["evidence_path"] = str(destination)
        atomic_json(destination, failure)
        atomic_json(candidate_destination, failure)
        atomic_json(Path(candidate["candidate_dir"]) / "status.json", candidate)
    ...
    def authorize_safe_infrastructure_repair(self, state: dict, candidate: dict, *, reason: str,
                                             test_evidence: dict) -> dict:
        """Authorize a bounded retry after an observational-only repair is tested."""
        if state.get("running_candidate"):
            raise RuntimeError("cannot authorize infrastructure repair while another candidate is running")
        if candidate.get("scientific_status") != "UNRESOLVED" or candidate.get("failure_classification") != "TELEMETRY_INFRASTRUCTURE_ERROR":
            raise RuntimeError("candidate is not an unresolved telemetry infrastructure failure")
        if not test_evidence.get("python_pass") or not test_evidence.get("rust_pass"):
            raise RuntimeError("infrastructure repair tests have not passed")
        maximum = int(self.config.payload["kaggle"]["max_infrastructure_retries"])
        used = int(candidate.get("infrastructure_retries", 0))
        if used >= maximum:
            candidate["status"] = "INFRASTRUCTURE_RETRY_EXHAUSTED"
            self.save_state(state)
            raise RuntimeError("infrastructure retry budget exhausted")
        previous = candidate.get("optimizer_telemetry_superseding_classification")
        repair = {
            "timestamp": utc_now(), "candidate_id": candidate["candidate_id"],
            "failure_class": "TELEMETRY_INFRASTRUCTURE_ERROR", "reason": reason,
            "scientific_status": "UNRESOLVED", "model_evidence_valid": False,
            "numerical_instability_established": False,
            "previous_classification_id": (previous or {}).get("classification_id"),
            "original_failure_preserved_at": str(self.paths.results / "failures" / f"{candidate['candidate_id']}.json"),
            "repair_scope": "TELEMETRY_ONLY", "test_evidence": test_evidence,
            "retry_number": used + 1, "max_infrastructure_retries": maximum,
        }
        candidate.update({
            "status": "INFRASTRUCTURE_RETRY_PENDING", "scientific_status": "UNRESOLVED",
            "failure_class": "TELEMETRY_INFRASTRUCTURE_ERROR",
            "failure_classification": "TELEMETRY_INFRASTRUCTURE_ERROR",
            "failure_reason": reason, "model_evidence_valid": False,
            "numerical_instability_established": False,
            "retry_eligible": False, "infrastructure_retries": used + 1,
            "safe_infrastructure_repair": repair, "updated_at": utc_now(),
        })
        state["budget"]["infrastructure_retries_consumed"] = state["budget"].get("infrastructure_retries_consumed", 0) + 1
        atomic_json(Path(candidate["candidate_dir"]) / f"infrastructure_repair_{used + 1}.json", repair)
        atomic_json(Path(candidate["candidate_dir"]) / "status.json", candidate)
        self.save_state(state); self.update_leaderboard(state)
        return repair
    ...
    def ingest_stability_result(self, state: dict, candidate: dict, source: str | Path, summary: dict | None = None) -> bool:
        ...
        primary_nonfinite = extract_first_nonfinite_evidence(summary)
        if primary_nonfinite:
            summary = {**summary, **primary_nonfinite}
        ...
        if not passed:
            primary_exception = summary.get("primary_exception")
            self.record_failure(
                state, candidate, "MULTI_EPOCH_STABILITY",
                primary_exception or summary.get("exception") or "; ".join(reasons), summary,
        ...
        if not state.get("first_validated_transformer"):
            milestone = {
                "status": "FIRST_VALIDATED_TRANSFORMER_READY",
                "candidate_id": candidate["candidate_id"],
                "candidate_key": candidate["candidate_key"],
                "checkpoint": str(Path(candidate["checkpoint_dir"]) / "best.pt"),
                "checkpoint_sha256": manifest["sha256"],
                "checkpoint_size": manifest["size"],
                "finite_parameters": bool(summary.get("finite_parameters")),
                "finite_optimizer_state": bool(summary.get("finite_optimizer_state")),
                "checkpoint_load": bool(summary.get("checkpoint_load")),
                "deterministic_inference": bool(summary.get("deterministic_inference")),
                "lossless_sha256": bool(summary.get("lossless_sha256")),
                "validated_at": utc_now(),
            }
            state["first_validated_transformer"] = milestone
            candidate["delivery_milestone"] = "FIRST_VALIDATED_TRANSFORMER"
            atomic_json(self.paths.results / "first_validated_transformer.json", milestone)
    ...
    def _check_stop_conditions(self,state:dict)->None:
        budget=state["budget"]
        if self.search_budget_exhausted(state):
            self.stop(state,"SEARCH_BUDGET_EXHAUSTED")
        elif state.get("first_validated_transformer") and budget["eligible_without_improvement"]>=budget["no_improvement_patience"]:self.stop(state,"NO_SIGNIFICANT_IMPROVEMENT")
    ...
    def search_budget_exhausted(self, state: dict) -> bool:
        budget = state["budget"]
        return any((
            budget["gpu_hours_consumed"] >= budget["max_gpu_hours"],
            budget["candidates_consumed"] >= budget["max_candidates"],
            budget["kaggle_submissions_consumed"] >= budget["max_kaggle_submissions"],
            budget["failures_consumed"] >= budget["max_failures"],
        ))

    @staticmethod
    def _candidate_is_terminal(candidate: dict) -> bool:
        return candidate.get("status") in TERMINAL_STATUSES | {"VALIDATED", "PROMOTED"}

    def _pending_candidate_is_eligible(self, state: dict, key: str, records: dict[str, dict]) -> bool:
        """Apply only the dependencies declared by the bounded candidate plan."""
        definition = self.config.candidates[key]
        parent = definition["parent"]
        rule = definition.get("activation_rule", "")
        if key == "lr_0_0003":
            return not records
        if key == "lr_0_0002":
            first = records.get("lr_0_0003")
            return bool(first and self._candidate_is_terminal(first))
        if key == "residual_scale_depth" or rule == "both_lr_candidates_fail_residual_growth":
            lr_candidates = [records.get("lr_0_0003"), records.get("lr_0_0002")]
            both_terminal = all(item and self._candidate_is_terminal(item) for item in lr_candidates)
            instability_measured = any(
                item and item.get("failure_classification") in NUMERICAL_FAILURES
                for item in lr_candidates
            )
            return bool(both_terminal and instability_measured)
        if rule == "stabilization_fallback_after_residual_failure":
            parent_record = records.get(parent)
            return bool(parent_record and self._candidate_is_terminal(parent_record))
        if rule == "final_safe_training_fallback":
            parent_record = records.get(parent)
            return bool(parent_record and self._candidate_is_terminal(parent_record))
        if parent == "@best_stable" or rule == "stable_candidate_quality_search":
            return bool(state.get("best_stable_candidate"))
        if parent == "base":
            return rule == "always"
        parent_record = records.get(parent)
        return bool(parent_record and parent_record.get("status") in {"STABILITY_VALIDATED", "VALIDATED", "PROMOTED"})

    def eligible_pending_candidate_keys(self, state: dict) -> list[str]:
        records = {item["candidate_key"]: item for item in state["candidates"].values()}
        pending = [
            key for key in state.get("pending_candidates", [])
            if key in self.config.candidates and key not in records
        ]
        eligible = [key for key in pending if self._pending_candidate_is_eligible(state, key, records)]
        # Stabilization precedes capacity expansion after measured instability.
        priority = {"residual_scale_depth": 0, "residual_scale_conservative": 1,
                    "safe_training_lr_0_0001": 2, "context_512": 3, "ffn_1024": 4, "layers_6": 5}
        config_order = {item["key"]: index for index, item in enumerate(self.config.payload["candidates"])}
        return sorted(eligible, key=lambda key: (priority.get(key, 0), config_order[key]))

    def reactivate_search_if_pending(self, state: dict) -> bool:
        """Recover an obsolete NO_VALID_CANDIDATE stop when approved work remains."""
        if state.get("status") != "NO_VALID_CANDIDATE":
            return False
        if self.search_budget_exhausted(state):
            self.stop(state, "SEARCH_BUDGET_EXHAUSTED")
            self.save_state(state)
            return False
        resumable = any(
            item.get("status") in {"PENDING", "INFRASTRUCTURE_RETRY_PENDING"}
            for item in state["candidates"].values()
        )
        if not resumable and not self.eligible_pending_candidate_keys(state):
            return False
        state["status"] = "ACTIVE"
    ...
    def next_candidate_key(self,state:dict)->str|None:
        ...
        if self.search_budget_exhausted(state):
            ...
            self.save_state(state)
            return None
        if state["status"] not in {"ACTIVE", "NO_VALID_CANDIDATE"}:
            return None
        eligible = self.eligible_pending_candidate_keys(state)
        if eligible and state["status"] == "NO_VALID_CANDIDATE":
            state["status"] = "ACTIVE"
            state["stop_reason"] = None
            self.save_state(state)
