...
def _collection_abs_max(values, device):
    maxima = [value.detach().float().abs().max() for value in values if torch.is_tensor(value) and value.numel()]
    return torch.stack(maxima).max() if maxima else torch.zeros((), device=device)
...
def train(
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        for layer in range(cfg.num_layers):
            ...
            activation_fields[f"block_{layer}_activation_abs_max"] = float(torch.stack(block_maxima).max().item()) if block_maxima else 0.0
