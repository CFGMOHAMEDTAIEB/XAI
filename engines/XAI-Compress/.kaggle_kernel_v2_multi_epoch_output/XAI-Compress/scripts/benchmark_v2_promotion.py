...
def main() -> None:
    ...
    if old_summary and new_summary:
        ...
        if high < 0 and relative >= .01:decision="PROMOTE V2"
        elif low > 0 and relative <= -.01:decision="KEEP OLD GRU"
    report={"decision":decision,"paired_files":len(paired),"paired_mean_delta_bpb":statistics.mean(paired) if paired else None,"paired_bootstrap_95pct_ci":[low,high],"minimum_material_improvement":.01,"summaries":summaries}
