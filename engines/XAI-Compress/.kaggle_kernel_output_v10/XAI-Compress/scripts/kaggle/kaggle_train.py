...
def run(dataset: Path, preset="BALANCED", resume=None, overrides=None, label="best"):
    ...
    try:
        ...
        status_path.write_text(json.dumps({"status":"FAILED", "exception_type":type(exc).__name__,
            "exception":str(exc), "last_update_timestamp":datetime.now(timezone.utc).isoformat()}, indent=2), encoding="utf-8")
        raise
