            ...
            if step % max(1, grad_accum) == 0:
                ...
                if numerical_debug:
                    for name,p in (model.module if hasattr(model,"module") else model).named_parameters():
                        if p.grad is not None:require_finite(p.grad,f"gradient:{name}",epoch,step)
                grad_norm = nn.utils.clip_grad_norm_((model.module if hasattr(model, "module") else model).parameters(), 1.0)
                last_gradient_norm = float(grad_norm.detach())
                if numerical_debug:
                    require_finite(grad_norm,"total_gradient_norm",epoch,global_step)
                    for name,p in (model.module if hasattr(model,"module") else model).named_parameters():
                        if p.grad is not None:require_finite(p.grad,f"clipped_gradient:{name}",epoch,global_step)
                scaler.step(opt)
                scaler.update()
                if numerical_debug:
                    for name,p in (model.module if hasattr(model,"module") else model).named_parameters():
                        require_finite(p,f"parameter:{name}",epoch,step)
                    for state in opt.state.values():
                        for key,value in state.items():
                            if torch.is_tensor(value):require_finite(value,f"optimizer:{key}",epoch,step)
                opt.zero_grad(set_to_none=True)
