...
class CausalByteTransformerV2(nn.Module):
    ...
    def forward(self,tokens:torch.Tensor,hidden:Any=None):
        ...
        # The output projection was the first repeatedly failing gradient in
        # long T4 runs. Keep normalization and logits in FP32 while attention
        # and FFN matrix multiplies remain AMP/tensor-core eligible.
        with torch.autocast(device_type=x.device.type, enabled=False):
            x=self.norm(x.float())
            if self.numerical_debug:_finite("final_norm",x)
            logits=torch.nn.functional.linear(x, self.output.weight.float())
