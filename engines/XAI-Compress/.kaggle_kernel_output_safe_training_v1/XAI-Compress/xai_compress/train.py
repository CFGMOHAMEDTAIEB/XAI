...
def reduced_amp_scale(scale: float, factor: float = 0.5, minimum: float = 1.0) -> float:
    """Return the bounded scale used after a recoverable AMP overflow."""
    value = float(scale)
    if not np.isfinite(value) or value <= 0:
        raise ValueError("AMP scale must be finite and positive")
    return max(float(minimum), value * float(factor))
...
def train(
    ...
    safe_training_mode=False,
    max_amp_recoveries=3,
    max_checkpoint_rollbacks=1,
    ...
    last_stable_path = output_path.with_name("last_stable" + output_path.suffix)
    recovery_path = output_path.with_suffix(".recovery.jsonl")
    ...
    def record_recovery(**event):
        payload = {"timestamp": time.time(), "global_step": global_step, **event}
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        for step, (x, y) in enumerate(train_loader, start=1):
            ...
            try:
                with _autocast(device, use_amp):
                    logits, _ = model(x)
                loss = criterion(logits.float().reshape(-1, 256), y.reshape(-1)) / max(1, grad_accum)
            ...
            try:
                ...
                if step % max(1, grad_accum) == 0:
                    ...
                    gradients_finite, _ = collection_finite_and_abs_max(gradients)
                    if not gradients_finite and safe_training_mode and use_amp:
                        old_scale = float(scaler.get_scale())
                        new_scale = reduced_amp_scale(old_scale)
                        opt.zero_grad(set_to_none=True)
                        scaler.update(new_scale=new_scale)
                        amp_recoveries += 1
                        consecutive_amp_overflows += 1
                        record_recovery(event="AMP_GRADIENT_OVERFLOW", action="SKIP_OPTIMIZER_STEP",
                                        epoch=epoch, step=step, old_scale=old_scale, new_scale=new_scale,
                                        consecutive=consecutive_amp_overflows)
                        if consecutive_amp_overflows <= int(max_amp_recoveries):
                            continue
                        if last_stable_path.is_file() and checkpoint_rollbacks < int(max_checkpoint_rollbacks):
                            restored, stable = load_checkpoint(last_stable_path, device)
                            trainable.load_state_dict(restored.state_dict())
                            opt.load_state_dict(stable["optimizer_state"])
                            new_lr = max(float(minimum_learning_rate), float(opt.param_groups[0]["lr"]) * 0.5)
                            for group in opt.param_groups:
                                group["lr"] = new_lr
                            if stable.get("scheduler_state"):
                                scheduler.load_state_dict(stable["scheduler_state"])
                            if stable.get("scaler_state"):
                                scaler.load_state_dict(stable["scaler_state"])
                            scaler.update(new_scale=min(float(scaler.get_scale()), new_scale))
                            checkpoint_rollbacks += 1
                            consecutive_amp_overflows = 0
                            record_recovery(event="REPEATED_AMP_OVERFLOW", action="RESTORE_LAST_STABLE_AND_REDUCE_LR",
                                            epoch=epoch, step=step, learning_rate=new_lr,
                                            checkpoint=str(last_stable_path), rollback=checkpoint_rollbacks)
                            continue
                        raise FloatingPointError(
                            f"UNRECOVERABLE REPEATED NON-FINITE GRADIENT; epoch={epoch}; step={step}"
                        )
                    if numerical_debug:
        ...
        with torch.inference_mode():
            for x, y in val_loader:
                ...
                try:
                    with _autocast(device, use_amp):
                        logits, _ = model(x)
                    loss = criterion(logits.float().reshape(-1, 256), y.reshape(-1))
                except FloatingPointError as exc:
        ...
        tracked = {**row, "best_validation_cross_entropy": min(best, va), "best_validation_bpb": min(best, va) / np.log(2),
                   "global_step": global_step, "amp_recoveries": amp_recoveries,
        ...
        parameters_finite, _ = collection_finite_and_abs_max(core.parameters())
        optimizer_finite, _ = collection_finite_and_abs_max(opt.state_dict())
        if not parameters_finite or not optimizer_finite or not all(np.isfinite(v) for v in (tr, va)):
            raise FloatingPointError(f"last-stable gate failed at epoch {epoch}")
        save_checkpoint(last_stable_path, core, opt, epoch, {**tracked, "checkpoint_role": "last_stable"},
                        scaler=scaler if use_amp else None, scheduler=scheduler)
        load_checkpoint(last_stable_path, "cpu")
        ...
        json.dumps(
            {
                ...
                "seed": seed,
                "device": device,
                "safe_training_mode": bool(safe_training_mode),
                "amp_recoveries": amp_recoveries,
                "checkpoint_rollbacks": checkpoint_rollbacks,
                "last_stable_checkpoint": str(last_stable_path),
            },
            indent=2,
        ),
        encoding="utf-8",
...
def build_parser():
    ...
    parser.add_argument("--safe-training-mode", action=argparse.BooleanOptionalAction, default=False)
    parser.add_argument("--max-amp-recoveries", type=int, default=3)
    parser.add_argument("--max-checkpoint-rollbacks", type=int, default=1)
