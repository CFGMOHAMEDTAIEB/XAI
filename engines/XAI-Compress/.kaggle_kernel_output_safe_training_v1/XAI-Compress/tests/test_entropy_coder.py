...
def test_randomized_rans_varying_distribution_roundtrip():
    import random
    import torch
    rng = random.Random(20260831)
    symbols = [rng.randrange(256) for _ in range(500)]
    tables = [logits_to_cumulative(torch.randn(256, generator=torch.Generator().manual_seed(i)))
              for i in range(len(symbols))]
    encoder = RANSEncoder()
    for table, symbol in zip(tables, symbols):
        encoder.write(table, symbol)
    decoder = RANSDecoder(encoder.finish())
    assert [decoder.read(table) for table in tables] == symbols
