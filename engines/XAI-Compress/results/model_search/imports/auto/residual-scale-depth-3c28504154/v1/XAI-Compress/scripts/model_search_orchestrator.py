...
from xai_compress.autonomous import AutonomousController
...
def main()->None:
    parser=argparse.ArgumentParser();parser.add_argument("command",choices=("start","resume","status","leaderboard","benchmark-best","ingest-diagnostic","register-next","mark-external","verify-baseline","reclassify-candidate-a","auto"));parser.add_argument("--config",type=Path,default=DEFAULT_CONFIG);parser.add_argument("--result",type=Path);parser.add_argument("--candidate");parser.add_argument("--kernel");parser.add_argument("--version",type=int);parser.add_argument("--once",action="store_true");parser.add_argument("--interval",type=int,default=45);args=parser.parse_args()
    if args.command=="auto":
        result=AutonomousController(args.config,poll_seconds=args.interval).run(once=args.once)
        print(json.dumps({"result":result},indent=2));return
