...
## Controlled neural-lossless model search

The resumable orchestrator uses `configs/model_search_v2.json` and stores its
atomic state under `results/model_search/`. It never changes
`checkpoints/kaggle/best.pt`, never treats validation BPB as artifact BPB, and
does not replace the application checkpoint automatically.

```powershell
.\.venv\Scripts\python.exe scripts\model_search_orchestrator.py start
.\.venv\Scripts\python.exe scripts\model_search_orchestrator.py resume
.\.venv\Scripts\python.exe scripts\model_search_orchestrator.py status
.\.venv\Scripts\python.exe scripts\model_search_orchestrator.py leaderboard
.\.venv\Scripts\python.exe scripts\model_search_orchestrator.py benchmark-best
```

Downloaded diagnostics are ingested without retraining:

```powershell
.\.venv\Scripts\python.exe scripts\model_search_orchestrator.py ingest-diagnostic `
  --result .kaggle_kernel_output\results\model_search
```

Every candidate has a deterministic ID and isolated checkpoint namespace.
Failures retain exact JSON evidence. A stability-validated Transformer must
still pass local checkpoint loading, finite parameter and optimizer scans,
deterministic inference, SHA-256 XAIC round trips, and the paired actual
compression benchmark before it is eligible for promotion. Missing codec or
measurement values remain `N/A`.
