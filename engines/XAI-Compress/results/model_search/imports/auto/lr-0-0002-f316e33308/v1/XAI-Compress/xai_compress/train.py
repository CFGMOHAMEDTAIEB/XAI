...
def _collection_abs_max(values, device):
    # AdamW contains CPU step counters alongside CUDA moment tensors. Reduce
    # each scalar on its own device before creating one diagnostic result;
    # stacking/catenating the mixed-device telemetry is invalid.
    maxima = [
        float(value.detach().float().abs().max().item())
        for value in values
        if torch.is_tensor(value) and value.numel()
    ]
    return torch.tensor(max(maxima, default=0.0), device=device)
...
def aggregate_diagnostic_scalars(values) -> float:
    ...
    for value in values:
        ...
        if torch.is_tensor(value):
            if value.numel() != 1:
                raise ValueError("diagnostic aggregation accepts scalar tensors only")
            value = value.detach().item()
        numeric = float(value)
        if np.isfinite(numeric):
            collected.append(numeric)
    return max(collected, default=0.0)
...
def train(
        ...
        def finite_and_abs_max(values):
            ...
            for value in values:
                ...
                if bool(mask.any().item()):
                    maxima.append(detached[mask].abs().max())
            maximum = float(torch.stack(maxima).max().item()) if maxima else None
            return finite, maximum

        parameters_finite, parameter_max = finite_and_abs_max(parameters)
        gradients_finite, gradient_max = finite_and_abs_max(gradients)
        optimizer_finite, optimizer_max = finite_and_abs_max(optimizer_tensors)
        gradient_norm = None
        if gradients:
            candidate = torch.linalg.vector_norm(torch.stack([gradient.detach().float().norm() for gradient in gradients]))
            if bool(torch.isfinite(candidate).item()):
                gradient_norm = float(candidate.item())
        return {
            "loss": current_loss,
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        model.train()
        total = n = 0
        epoch_gradient_abs_max = torch.zeros((), device=device)
        epoch_gradient_norm_max = torch.zeros((), device=device)
        epoch_gradient_norm_sum = torch.zeros((), device=device)
        ...
        for step, (x, y) in enumerate(train_loader, start=1):
            ...
            try:
                ...
                if step % max(1, grad_accum) == 0:
                    ...
                    epoch_gradient_abs_max = torch.maximum(epoch_gradient_abs_max, step_gradient_abs_max)
                    grad_norm = nn.utils.clip_grad_norm_(trainable.parameters(), 1.0)
                    last_gradient_norm = float(grad_norm.detach())
                    epoch_gradient_norm_max = torch.maximum(epoch_gradient_norm_max, grad_norm.detach().float())
                    epoch_gradient_norm_sum = epoch_gradient_norm_sum + grad_norm.detach().float()
        ...
        row = {
            ...
            "gradient_norm_epoch_max": float(epoch_gradient_norm_max.item()),
            "gradient_norm_epoch_mean": float((epoch_gradient_norm_sum / max(1, epoch_gradient_norm_count)).item()),
            "gradient_abs_max": float(epoch_gradient_abs_max.item()),
            "parameter_abs_max": float(parameter_abs_max.item()),
            "optimizer_state_abs_max": float(optimizer_state_abs_max.item()),
