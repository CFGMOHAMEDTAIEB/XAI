...
$Metadata=@{id="$($env:KAGGLE_USERNAME)/xai-compress-lossless-v2-cuda-diagnostic";title="XAI Compress Lossless V2 CUDA Diagnostic";code_file="run.py";language="python";kernel_type="script";is_private=$true;enable_gpu=$true;machine_shape="NvidiaTeslaT4";enable_internet=$false;dataset_sources=@("$($env:KAGGLE_USERNAME)/xai-compress-source","xdxd003/ff-c23");competition_sources=@();kernel_sources=@()} | ConvertTo-Json -Depth 4
