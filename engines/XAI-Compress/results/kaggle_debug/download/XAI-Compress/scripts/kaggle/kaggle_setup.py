...
def dataset_stats(root: Path) -> dict:
    ...
    return {"path": str(root), "files": len(files), "bytes": sum(s for _, s in sizes), "extensions": dict(Counter(p.suffix.lower() or "<none>" for p, _ in sizes)), "largest": [{"path": str(p), "bytes": s} for p, s in sorted(sizes, key=lambda x: x[1], reverse=True)[:10]]}
...
def setup(source=None, destination=Path("/kaggle/working/XAI-Compress")):
    ...
    check_dependencies()
    subprocess.run([sys.executable, "-m", "pip", "install", "-e", str(destination), "--no-deps"], check=True)
    return destination
