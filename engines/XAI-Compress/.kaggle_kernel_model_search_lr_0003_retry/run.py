"""Single authorized clean retry for Candidate A (LR=0.0003)."""
...
RESULTS = WORK / "results" / "model_search" / "lr_0_0003"
...
print(json.dumps({
    "candidate": "lr_0_0003", "learning_rate": LEARNING_RATE,
    "retry_number": 1, "gpu": torch.cuda.get_device_name(0),
...
print("STARTING CANDIDATE A CLEAN RETRY ONLY", json.dumps(command), flush=True)
...
selection = {
    ...
    "candidate_a_retrained": True,
    "candidate_a_retry_number": 1,
}
(RESULTS.parent / "selection.json").write_text(json.dumps(selection, indent=2), encoding="utf-8")
print("CANDIDATE A RETRY SELECTION", json.dumps(selection), flush=True)
