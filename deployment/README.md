...
1. **Configure Brevo.** Create/verify the sender in Brevo and obtain a transactional API key. For `EMAIL_PROVIDER=brevo`, set `BREVO_API_KEY`, `BREVO_SENDER_EMAIL`, and `BREVO_SENDER_NAME`; SMTP variables are not consumed by the Brevo HTTPS transport. HTTPS uses `https://api.brevo.com/v3/smtp/email`, authenticated with the api-key header. SMTP remains available only with `EMAIL_PROVIDER=smtp`, `SMTP_HOST=smtp-relay.brevo.com`, the SMTP username/key, and provider-supported `SMTP_PORT`/`SMTP_SECURITY`. API keys and SMTP keys are different credentials. Confirm current account quotas/transactional activation in Brevo. Free-tier availability does not establish delivery.
...
2. **Deploy backend.** Create Render PostgreSQL and a Docker web service in the same region. Backend build root is repository root (Root Directory empty), Dockerfile `services/api_fastapi/Dockerfile`. Import .env.render.api. Convert the internal PostgreSQL URL scheme to `postgresql+psycopg://` for the installed psycopg 3 driver; preserve encoded credentials and any URL options. Generate a random JWT_SECRET locally, at least 32 characters. Health check is `/health`. The Docker command first runs `python scripts/run_all_migrations.py --apply` and starts Uvicorn only when that command succeeds. Render's PORT is honored; template uses 10000. Local Compose uses 8000.
   ...
   Attach a persistent disk at `/data`. The existing code stores both uploaded files and compressed artifacts there. Render free web services have ephemeral filesystems and no persistent disks. Brevo HTTPS avoids blocked SMTP ports but does not solve persistence. Use a single persistent-disk instance for the current architecture. Production startup does not call SQLAlchemy `create_all`; the original base schema must already be provisioned. The automatic runner discovers numbered SQL files, serializes competing instances with a PostgreSQL advisory lock, records filenames and SHA-256 checksums in `schema_migrations`, transactionally applies only pending migrations, and blocks API startup on a migration or checksum failure. On its first tracked startup it recognizes complete legacy migrations 001-004 from catalog metadata and records them without re-running their SQL.

   A read-only preflight can be run in a backend shell with `python scripts/run_all_migrations.py --check-only`. It exits non-zero when migrations are pending or migration tracking/checksums are invalid. Never edit an already-applied migration; add the next numbered migration instead.
