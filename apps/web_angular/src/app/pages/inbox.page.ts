...
<article class="panel form-panel"><label>Share code<input [(ngModel)]="code" placeholder="XC-ABCD-EFGH-IJKL"></label><button class="primary" [disabled]="!code" (click)="redeem()">Redeem</button>@if(result()){<button (click)="downloadShare()">Download shared artifact</button><pre>{{result()|json}}</pre>}</article>`})
export class InboxPage{
  sharedName='shared.xaic';code='';file:File|null=null;busy=signal(false);message=signal('');result=signal<unknown>(null);
  ...
  downloadShare(){this.api.downloadShare(this.code).subscribe({next:b=>saveBlob(b,this.sharedName),error:()=>this.message.set('Shared artifact unavailable or download limit reached.')})}
  redeem(){this.api.redeem(this.code).subscribe({next:x=>{this.result.set(x);this.sharedName=(x as {file:{name:string}}).file.name+'.xaic'},error:e=>this.result.set({error:e?.error?.detail??'Backend unavailable'})})}
}
